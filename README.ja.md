...
複数の AI エージェント（claude code、codex、hermes、gajaecode、lettacode など）のコンテキスト／メモリを、単一の共有ローカルストアに統合する v1 システム。

**一行のゴール:** どのエージェントで決めた決定・好みも、再説明なしにすべてのエージェントの新しいセッションに引き継がれる。

## アーキテクチャ（正本 = store-host、デバイス非依存アクセス）
...
- デバイス非依存: 正本ストアは store-host — Mac などの他マシンは `uac.config.json` の ssh スポーンでアクセス（Phase 6）
...
| `docs/phase6-remote.md` | 正本ストアの store-host 移行 + ssh stdio-MCP アクセス |
