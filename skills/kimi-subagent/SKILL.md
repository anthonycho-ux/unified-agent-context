...
7. **sov server offload.** Heavy, long, unattended work goes to `ssh sov`
   (Ryzen 7, 61 GB RAM, always-on; no GPU).
   - `ssh sov '<command>'` runs a non-interactive, non-login bash shell, which
     reads none of `.bashrc`, `.bash_profile`, or `.profile`. On sov the real
   ...
   - Fix: always invoke as `ssh sov 'bash -lc "<command>"'`, never bare
     `ssh sov '<command>'`. No sov-side file changes needed or made; this is
