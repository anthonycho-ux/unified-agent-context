...
test('similarity: 동일 문장은 1, 무관 문장은 0', () => {
  ...
  assert.equal(similarity('kimi code cli is retired', 'stripe payments run on store-host'), 0);
});
...
test('verdict: 임계값 경계에서 duplicate / correction / new를 분류한다', () => {
  ...
  assert.equal(verdict('stripe payments run on store-host account', [base]).kind, 'new');
