...
async function withTwoStores(fn) {
  const dirSov = await fs.mkdtemp(path.join(os.tmpdir(), 'uac-union-sov-'));
...
test('union: merges both sources, dedupes by key, newer copy wins', async () => {
  await withTwoStores(async ({ sov, local }) => {
    await sov.storeFact(decision('sov only global fact', 'global'));
    await sov.storeFact(decision('shared project fact', 'project:x', '2026-01-01T00:00:00.000Z'));
    await sov.storeFact(decision('other project fact', 'project:z'));
    ...
    const result = await getInjectionBlock({ sovStore: sov, localStore: local, scope: 'project:x' });
    assert.equal(result.degraded, false);
    assert.equal(result.partial, false);

    const statements = result.facts.map((f) => f.statement).sort();
    assert.deepEqual(statements, ['local only global fact', 'shared project fact', 'sov only global fact']);
...
test('sov down: falls back to local-only, partial health, not degraded', async () => {
  await withTwoStores(async ({ local }) => {
    await local.storeFact(decision('survives sov outage', 'global'));
    const result = await getInjectionBlock({ sovStore: throwingStore, localStore: local, scope: 'global' });
    assert.equal(result.degraded, false);
    assert.equal(result.partial, true);
    assert.deepEqual(result.facts.map((f) => f.statement), ['survives sov outage']);
    const health = JSON.parse(await fs.readFile(process.env.UAC_HEALTH_PATH, 'utf8'));
    assert.equal(health.status, 'partial');
  });
});

test('local down: falls back to sov-only, partial health, not degraded', async () => {
  await withTwoStores(async ({ sov }) => {
    await sov.storeFact(decision('survives local outage', 'global'));
    const result = await getInjectionBlock({ sovStore: sov, localStore: throwingStore, scope: 'global' });
    assert.equal(result.degraded, false);
    assert.equal(result.partial, true);
