...
TL;DR: Basecamp 5 (2026) is agent-ready: official CLI, SDK, MCP server, and agent skills. Fleet account <basecamp-account-id> (Hermes Basecamp, <account-email>, Free plan, 3-project cap). CLI is installed on sov but NOT yet authenticated as of 2026-08-30; auth requires the user's manual Launchpad login (account password is not stored anywhere agent-accessible; Google is not linked to this account).
