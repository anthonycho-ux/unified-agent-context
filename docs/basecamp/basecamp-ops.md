...
TL;DR: Basecamp 5 (2026) is agent-ready: official CLI, SDK, MCP server, and agent skills. Fleet account <account-id> (fleet agent mailbox, Free plan, project cap). CLI is installed on sov but NOT yet authenticated as of 2026-08-30; auth requires a manual Launchpad login (account password is not stored anywhere agent-accessible; Google is not linked to this account).
...
## CLI (official, installed on sov)

- Binary: `/home/dev/bin/basecamp`, version **0.9.1** (install one-liner: `curl -fsSL https://basecamp.com/install-cli | bash`; brew: `brew install --cask basecamp/tap/basecamp-cli`)
...
- `basecamp auth status` on sov: **authenticated: false** as of 2026-08-30 ~01:00 MDT. Shell-out auth still required before any CLI Basecamp operation.
...
- 2026-08-26: account created via Hermes-initiated flow (launchpad OAuth, 127.0.0.1:8976 callback); CLI installed on sov (version file updated 17:05)
...
- Aug 26: Basecamp CLI version 0.9.1 installed on sov (`~/.config/basecamp/` directory created)
...
- 2026-09-09: Notification settings are WEB-ONLY (no CLI/API endpoint): app.basecamp.com/<account>/my/notifications/settings/edit — master Turn notifications off link kills all alerts instantly (email, pop-ups, reminders), verified live on the account; Turn notifications back on reverses. Also: CLI auth on sov is COMPLETE (verified 2026-09-09, launchpad OAuth, ~77h token TTL).
