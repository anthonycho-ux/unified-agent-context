...
store is an `mcp-memory-keeper` MCP server that lives on a self-hosted box **sov**, reached
...
## The problem
sov is reachable two ways: SSH alias `sov` (LAN) and `sov-ts` (Tailscale).
...
- Tailscale OFF at home: only LAN `sov` works; `sov-ts` times out.

Originally `uac.config.json` hardcoded a single `host: "sov"`, so whenever that one route was
...
- `uac.config.json` now has `"host": "sov-ts"`, `"fallbackHosts": ["sov"]` (sov-ts first
  because Tailscale is usually on; LAN sov as fallback).
...
## What to do
1. Confirm connectivity is currently up: `ssh -o BatchMode=yes -o ConnectTimeout=8 sov-ts hostname`
   (fall back to `sov`). If both time out, the tunnel/LAN is down — this is an environment
...
- Don't hardcode IPs in committed code; use SSH aliases (`sov`, `sov-ts`) and let ssh_config
  resolve them.
