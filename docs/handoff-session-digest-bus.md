...
3. Canonical persistence verified on sov:
   `grep -rl "strongest-critic" /home/dev/.uac/data/memory/` →
   `/home/dev/.uac/data/memory/context.db`.

The digest bus reuses exactly this pipeline (`src/recorder.mjs` →
`src/store-adapter.mjs` → ssh stdio-MCP `mcp-memory-keeper` on sov, per
`uac.config.json`: host `sov-ts`, fallback `sov`, dataDir
`/home/dev/.uac/data/memory`). What's new is a fact shape for digests, a
...
- Hooks (start with the two cheapest, stub the rest):
  ...
  - **Hermes / gjc / lettacode (sov-side)**: wrapper-script integration point;
    if their wrappers are out of reach this pass, mark stubbed in the README.
- Degraded mode matches existing convention: if sov is unreachable, warn on
  ...
  repo; the canonical store lives on sov outside the repo anyway).
...
- Stage-1 unified task ledger lives on sov:
...
- Storage: existing sov store; `summary` facts auto-expire via the existing
...
- No new datastore — reuse the sov mcp-memory-keeper store.
...
1. **Write + persist**: `record-digest.mjs --agent claude-code --session t1 "did: test"` exits 0, prints `RECORDED <key> <scope>`, and the statement is present in `/home/dev/.uac/data/memory/context.db` on sov within 60s.
...
4. **Degraded mode**: with sov unreachable (e.g. bogus `UAC_STORE_HOST` override), record-digest exits 0, warns on stderr, spools to `data/digest-outbox/`; next successful run flushes the spool and the digest appears in the store.
...
- `src/store-adapter.mjs` — ssh stdio-MCP to sov, `context_save`/`context_get`/`context_search`.
- `scripts/record-fact.mjs` / `scripts/inject-context.mjs` — CLI patterns to mirror.
- `uac.config.json` — store host resolution (`sov-ts` → `sov` fallback).
...
1. **task_id plumbing**: `UAC_TASK_ID` env-var convention needs a matching
   change in the Orca/Controller wrapper scripts on sov (out of this repo's
   scope) before ledger linkage is automatic; manual `--task` works meanwhile.
2. **Hermes/gjc/lettacode hooks**: sov-side wrapper edits are a separate
   dispatch; v1 may ship with only Claude Code/Codex hooks + Aside convention.
3. **Public-repo privacy**: resolved — store is on sov outside the repo and
