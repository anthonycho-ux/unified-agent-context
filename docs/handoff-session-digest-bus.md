...
1. **Write + persist**: `record-digest.mjs --agent claude-code --session t1 "did: test"` exits 0, prints `RECORDED <key> <scope>`, and the statement is present in `~/.uac/data/memory/context.db` on sov within 60s.
