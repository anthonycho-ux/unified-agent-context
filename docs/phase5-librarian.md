   ...
   └─▶ queueForLibrarian()  — 영구 팩트(decision/preference)만, fail-safe
         └─▶ data/memory/librarian/outbox.jsonl (dedupe: outbox+sent)
               └─▶ scripts/librarian-sync.mjs (수동/cron)
                     └─(ssh)─▶ store-host: Letta "The Noticer"
...
| Letta "The Noticer" (store-host) | **유일한 장기 기억 writer** — mem0 승격/병합/폐기 판단 (`librarian-role.md`) |
...
| `UAC_LIBRARIAN_HOST` | `store-host` | ssh 호스트 |
...
- 전체 스위트 73/73.
- 라이브 e2e (2026-07-08): `record-fact` → `librarian-sync` → store-host inbox에 digest 착지 확인. (교훈: async `execFile`은 `input` 미지원 → 원격 `cat` hang. `execFileSync`로 배달.)
