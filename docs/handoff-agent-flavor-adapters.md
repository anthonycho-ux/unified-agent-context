...
|---|---|---|---|
| Shared facts | UAC store on store-host (`mcp-memory-keeper`) | Yes | Agent-neutral claims only: preferences, decisions, project state |
...
- `docs/handoff-tailscale-connectivity.md` — store host fallback (store-host-ts → store-host) already applied; live failover not fully exercised
...
- No hardcoded IPs; use `store-host` / `store-host-ts` aliases
