...
(`store-host-ts` → `store-host` fallback).
...
Design detail: [`agent-flavor-adapters.md`](./agent-flavor-adapters.md).
...
| Store connection errors | `ssh -o ConnectTimeout=8 store-host-ts hostname` then `store-host`; see connectivity handoff. |
