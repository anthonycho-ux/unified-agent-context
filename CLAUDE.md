...
The canonical store is an `mcp-memory-keeper` MCP server on the self-hosted box **store-host**,
...
store-host is reachable via SSH alias `store-host` (LAN) or `store-host-ts` (Tailscale). The owner often leaves
Tailscale on and forgets to turn it off, so no single host is always reachable. UAC therefore
tries candidate hosts in priority order (`store-host-ts` → `store-host`) and uses the first that connects;
...
(`ssh -o ConnectTimeout=8 store-host-ts hostname`) before treating it as a bug.
