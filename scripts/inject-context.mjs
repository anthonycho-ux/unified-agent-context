...
// Usage: node scripts/inject-context.mjs [--agent generic|claude-code|codex|hermes|aside|gajaecode|lettacode] [--scope project:<id>|global] [--cwd <dir>]
...
function parseArgs(argv) {
  const args = { agent: 'generic', cwd: process.cwd(), scope: undefined };
...
const { agent, cwd, scope } = parseArgs(process.argv.slice(2));

try {
  const result = await getInjectionBlock({ agent, cwd, scope });
