...
LATEST=$(ssh -o ConnectTimeout=10 sov 'ls -t /home/dev/.uac/maintenance/reports/*-fitness.md 2>/dev/null | head -1')
if [ -z "${LATEST:-}" ]; then
  echo "no fitness reports on sov"
  exit 0
fi
...
scp -q -o ConnectTimeout=10 "sov:$LATEST" "$TMP/fitness.md" || { echo "scp fitness failed"; exit 1; }
scp -q -o ConnectTimeout=10 "sov:/home/dev/.uac/maintenance/reports/$DATE-cleanup.md" "$TMP/cleanup.md" 2>/dev/null || true

{
  echo "# UAC 주간 fitness — $DATE"
  echo
  echo "- 출처: sov 자동 점검 (매주 월요일 07:23, cron)"
  echo "- 원본: \`/home/dev/.uac/maintenance/reports/$BASE\` (sov)"
