...
// sov store. One pass = pull sov->local and push local->sov so the local store
...
//   - Local→sov normalization (Option A): local rows that intend to be facts
...
/**
 * Option A — local→sov normalization. A local raw row that *intends* to be a
...
export async function reconcile({ localStore, sovStore, now = () => new Date() } = {}) {
  ...
  for (const scope of scopes) {
    // Local→sov is the only normalization direction: local fact-intent rows are
...
async function connectSov() {
  ...
  for (const spec of specs) {
    try {
      ...
      // try the next candidate host (sov-ts -> sov)
    }
  }
  return null;
}
...
async function main() {
  ...
  try {
    ...
    if (!sovStore) {
      console.warn('[context-sync] no sov host reachable; skipping this cycle (will retry).');
      return;
    }
