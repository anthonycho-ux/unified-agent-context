...
- 设备无关：权威存储位于 store-host——其他机器（如 Mac）通过 `uac.config.json` 配置的 ssh 派生进程访问（Phase 6）
...
| `docs/phase6-remote.md` | 权威存储迁移至 store-host + ssh stdio-MCP 访问 |
